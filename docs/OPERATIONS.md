...
trusted `workflow_run` builds the dependency-update images, deploys only the
trusted `main` chart into the fixed `sre-dependabot-e2e` namespace, and runs
mock-AI/JSON browser coverage. Its kubeconfig is stored only in the unprotected
...
matching backend Pod on port 8080. PR-controlled images are built before the
workflow logs into GHCR; only the trusted default-branch workflow receives the
