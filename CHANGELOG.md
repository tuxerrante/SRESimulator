...
- Simplified the AKS `kubectl`-only enforcement. The root cause of the
  ...
  Correctness now rests on the scoped/CLI-neutral KB, the prompt, the `/command`
  HTTP 409 CLI guards (`isCommandTypeAllowedForPlatform` for the CLI type,
  `getCommandScopeViolation` for platform-incompatible resources), the
  frontend's non-runnable wrong-CLI block, and the live E2E gate as the
  regression net.
  ([#389](https://github.com/tuxerrante/SRESimulator/pull/389))
